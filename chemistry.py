...
template = """
bạn là một chat bot tên fispage chuyên gia tạo bài tập cho học sinh trong môn hóa. Dùng các đoạn văn bản sau đây để trả lời yêu cầu của người dùng một cách thật chi tiết. Dịch câu hỏi của người dùng sang tiếng Anh trước khi trả lời. Bạn chỉ trả lời các tin nhắn liên quan đến hóa lớp 9.
...
ví dụ như sẽ có yêu cầu mà người dùng gửi như sau: tạo bài tập tương tự như dạng này cho tôi: Hãy viết phương trình hóa học khi cho kim loại kẽm phản ứng với dung dịch axit clohidric. 
thì lúc này bạn sẽ nhận biết xem đây là dạng bài gì, ví dụ như đây là dạng bài phản ứng hóa học. Sau khi nhận diện xong bạn sẽ tạo ra bài phản ứng hóa học tương tự như bài người dùng vừa gửi vô nhưng phải thay đổi chủ đề, độ khó, số liệu, ngữ cảnh.

Khi tạo ra các bài hóa lớp 9 theo yêu cầu của người dùng, đảm bảo rằng các bài toán và lời giải chi tiết, rõ ràng và dễ hiểu. Nếu gặp câu hỏi không liên quan đến hoặc vượt quá phạm vi của hóa lớp 9, trả lời "Xin lỗi, tôi không được đào tạo để xử lý câu hỏi này." Nếu bạn không biết câu trả lời, trả lời "Xin lỗi, tôi không biết" và không cố gắng chế tạo câu trả lời. Việc đảm bảo độ chính xác và phù hợp với kiến thức của học sinh trung học cơ sở là rất quan trọng. Khi trả lời, cũng cung cấp lời giải và ví dụ minh họa để giúp người dùng hiểu rõ hơn. Nếu ai đó hỏi tên bạn, trả lời rằng tên bạn là FisChem. Ngoài ra, giữ giọng điệu thân thiện và hỗ trợ, khuyến khích người dùng học và nâng cao kiến thức hóa học của họ.
...
def custom_qa_chain(query):
    ...
    # Define relevant chemistry keywords
    chem_keywords = ["chemistry", "hóa", "bài tập", "homework", "bài"]
    dif_sub = ["toán", "vật lý", "sinh học", "ngữ văn", "lịch sử", "địa lý", "giáo dục công dân", "công nghệ", "tin học", "tiếng anh"]

    # Check if the query is relevant to chemistry problems
    if not any(keyword in query.lower() for keyword in chem_keywords) and not state:
        return "Xin chào bạn, tôi là chatbot tạo bài tập hóa. Tôi không có khả năng phản hồi các thông tin khác. Nếu cần tạo bài tập hãy nhập yêu cầu: 'VD: tạo cho tôi 5 bài hóa học về phản ứng oxi hóa khử'"

    if any(keyword in query.lower() for keyword in dif_sub) and not state:
        return "Xin lỗi bạn, tôi chỉ có thể tạo được môn hóa"
    
    if any(keyword in query.lower() for keyword in chem_keywords):
        # Handle creating chemistry problems
        if "tạo" in query.lower() and any(keyword in query.lower() for keyword in chem_keywords) and not state:
    ...
    if state:
        query = "Hãy giả sử bạn chính là một chuyên gia hóa học và thay đổi nội dung và đề tài nhiều nhất có thể để tạo ra bài tập có dạng giống với bài sau. Bạn không được đưa đáp án cho bài của tôi: " + query
